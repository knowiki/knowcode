...
* **Decision:** Single workspace monorepo named `knowcode` using `uv` as the package manager. `runtime` and `structural_engine` must be implemented as distinct, isolated top-level packages/modules within the same codebase.
