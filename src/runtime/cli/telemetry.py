...
def _send_telemetry_sync(command: str, status: str, project_id: str, access_key: str) -> None:
...
def send_telemetry_async(command: str, status: str) -> None:
    ...
    try:
        ...
        if not access_key or access_key == "dummy-access-key":
            return  # Skip telemetry if the user is not authenticated or in a test
